...
-- 【基础信息】
-- kpi
drop table if exists hive.mushroom2_tw_w.ads_kpi_hourly_hf;

create table if not exists hive.mushroom2_tw_w.ads_kpi_hourly_hf
(date date,
hour timestamp,
zone_id varchar,
channel varchar,
...
insert into hive.mushroom2_tw_w.ads_kpi_hourly_hf
...
a.role_id, a.pay_count, a.money, a.last_event,
...
count(distinct (case when money > 0 and retention_hour = 0 and pay_retention_hour = 0 then role_id else null end)) as install_pay,
count(distinct (case when money > 0 and pay_retention_hour = 0 then role_id else null end)) as newpay_users, 
...
data_cube as(
select distinct date, part_date, zone_id, channel, os, t.hour, date_format(t.hour, '%H') as hour_pure
from hourly_info a
