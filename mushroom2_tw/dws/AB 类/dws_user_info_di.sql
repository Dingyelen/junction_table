###
create table if not exists hive.mushroom2_tw_w.dws_user_info_di(
role_id varchar,
device_id varchar,
open_id varchar,
adid varchar,
...
delete from hive.mushroom2_tw_w.dws_user_info_di 
where exists(
select 1
...
drop table if exists hive.mushroom2_tw_w.temp_user_daily;

create table if not exists hive.mushroom2_tw_w.temp_user_daily as
select a.*, 
...
where exists
(select 1
from hive.mushroom2_tw_w.dws_user_daily_di z
where part_date >= $start_date
and  part_date <= $end_date
and a.role_id = z.role_id
);

drop table if exists hive.mushroom2_tw_w.temp_user_info;

create table if not exists hive.mushroom2_tw_w.temp_user_info as
select role_id, 
...
from hive.mushroom2_tw_w.temp_user_daily 
group by 1;

drop table if exists hive.mushroom2_tw_w.temp_user_first_info;

create table if not exists hive.mushroom2_tw_w.temp_user_first_info as
...
insert into hive.mushroom2_tw_w.dws_user_info_di(
...
from hive.mushroom2_tw_w.temp_user_info a
left join hive.mushroom2_tw_w.temp_user_first_info b
