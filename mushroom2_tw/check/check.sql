...
select role_id, count(1) FROM hive.mushroom2_tw_w.dws_user_daily_di
where part_date = date_format(date_add('day', -1, current_date), '%Y-%m-%d'))
group by 1
HAVING count(1) > 1
...
from hive.mushroom2_tw_w.dws_user_daily_derive_di
...
check_data as (
select count(distinct role_id) as users
from hive.mushroom2_tw_w.dws_user_hourly_hi
where part_date >= date_format(date_add('day', -1, current_date), '%Y-%m-%d')
and part_date <= date_format(current_date, '%Y-%m-%d')
...
from hive.mushroom2_tw_r.dwd_gserver_corechange_live
where part_date = date_format(date_add('day', -1, current_date), '%Y-%m-%d')
and reason != '638'),
...
from hive.mushroom2_tw_w.ads_active_daily_di
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
group by 1)
...
with base_log as(
select a.*
from hive.mushroom2_tw_w.dws_core_snapshot_di a
left join hive.mushroom2_tw_w.dim_gserver_base_roleid b
on a.role_id = b.role_id
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
...
check_data as(
select part_date, sum(core_add) as core_add
from hive.mushroom2_tw_w.ads_core_addreason_di
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
group by 1)
...
check_data as(
select part_date, sum(core_cost) as core_cost
from hive.mushroom2_tw_w.ads_core_costreason_di
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
group by 1)
...
check_data as(
select part_date, sum(dau) as users
from hive.mushroom2_tw_w.ads_retention_daily_di
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
and retention_day = 0
group by 1)
...
from hive.mushroom2_tw_w.ads_user_retention_di
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
and retention_day = 0
group by 1)
...
check_data as(
select part_date, cast(sum(money) as bigint) as money
from hive.mushroom2_tw_w.ads_kpi_daily_di
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
group by 1)
...
check_data as(
select part_date, sum(dau) as users
from hive.mushroom2_tw_w.ads_kpi_hourly_hi
where part_date >= date_format(date_add('day', -30, current_date), '%Y-%m-%d')
group by 1)
