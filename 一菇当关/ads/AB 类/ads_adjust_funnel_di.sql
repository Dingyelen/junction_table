create table if not exists hive.mushroom_tw_w.ads_adjust_funnel_di(
funnel_type varchar,
...
delete from hive.mushroom_tw_w.ads_adjust_funnel_di 
where part_date >= $start_date and part_date <= $end_date;

insert into hive.mushroom_tw_w.ads_adjust_funnel_di(
...
from hive.mushroom_tw_w.dws_user_info_di a
left join hive.mushroom_tw_w.dws_user_info2_di b
...
from hive.mushroom_tw_r.dwd_adjust_live a
left join hive.mushroom_tw_w.dim_adjust_loading_adjusteventname b
...
from hive.mushroom_tw_r.dwd_gserver_guidestep_live a
left join hive.mushroom_tw_w.dim_gserver_guidestep_stepid b
...
from 
(select distinct event_name, step_num, union_step from hive.mushroom_tw_w.dim_adjust_loading_adjusteventname where is_compulsory = '1')
cross join 
...
from 
(select distinct step_id, step_num, union_step from hive.mushroom_tw_w.dim_gserver_guidestep_stepid where is_compulsory = '1')
cross join 
