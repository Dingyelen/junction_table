...
from hive.mushroom_tw_r.dwd_merge_battle_live
where part_date >= $start_date
and part_date <= $end_date
...
from hive.mushroom_tw_r.dwd_gserver_challengechallenge_live
where part_date >= $start_date
and part_date <= $end_date)
union all(
select date(event_time) as date, part_date, role_id
from hive.mushroom_tw_r.dwd_gserver_instancebattle_live
where part_date >= $start_date
and part_date <= $end_date)
), 
